...
Game.prototype.clickSelect = function(add) {
    ...
    if (e.faction === 0) {
      if (this.registerSelectionTap && this.registerSelectionTap(e, add)) return;
      if (add) {
...
Game.prototype.pickEntity = function(x, y) {
  for (let i = this.units.length - 1; i >= 0; i--) {
    ...
    if (!unit.dead && !unit.garrisoned && dist2(x, y, unit.x, unit.y) <= (unit.r + 7) * (unit.r + 7)) return unit;
  }

  for (let i = this.buildings.length - 1; i >= 0; i--) {
    const building = this.buildings[i];
    if (!building.dead && Math.abs(x - building.x) <= building.w * .62 && Math.abs(y - building.y) <= building.h * .62) return building;
  }

  for (let i = this.resources.length - 1; i >= 0; i--) {
    const resource = this.resources[i];
    if (resource.dead) continue;
    if (resource.amount <= 0 && !resource.depleted) continue;
    const spec = getResourceVisualSpec(resource);
    const visualCenterY = resource.y - ((spec.baseline || spec.fh || 0) * (spec.scale || 1)) * .48;
    const visualRadius = Math.max(resource.r + 10, Math.min(58, (spec.fh || resource.r * 2) * (spec.scale || 1) * .38));
    if (dist2(x, y, resource.x, resource.y) <= (resource.r + 9) * (resource.r + 9)
      || dist2(x, y, resource.x, visualCenterY) <= visualRadius * visualRadius) return resource;
  }

  for (let i = this.decor.length - 1; i >= 0; i--) {
    const decor = this.decor[i];
    if (decor.dead || decor.sky || decor.water) continue;
    const spec = DECOR_SPECS[decor.kind] || {};
    const width = (spec.fw || 64) * (decor.scale || 1) * .55;
    const height = (spec.fh || 64) * (decor.scale || 1) * .55;
    const centerY = decor.y - ((spec.baseline || spec.fh || 64) * (decor.scale || 1) - height) * .35;
    if (dist2(x, y, decor.x, centerY) <= Math.max(width, height) * Math.max(width, height)) return decor;
  }
  return null;
};
...
Game.prototype.cancelModes = function() {
  ...
  if (HUD.buildMenu) HUD.buildMenu.classList.add('hidden');
  if (HUD.help) HUD.help.classList.add('hidden');
  this.uiDirty = true;
};
